...
import {
  computeEmbeddings,
  docQueryToQueryDoc,
  docsToParallelArrays,
  parallelArraysToDocs,
} from "./Documents";
...
export class ChromaClient {
  ...
  /**
   * Gets a collection with the specified properties. If one does not already
   ...
   * ```
   */
  public async addDocuments(collection: Collection, documents: ChromaDoc[]) {
    await this.initPromise;
    return await this.api
      .add(
        collection.id,
  ...
  public async getDocuments(
    ...
    return parallelArraysToDocs({
      ids: response.ids,
      embeddings: response.embeddings ?? undefined,
      documents: response.documents as string[] | undefined,
